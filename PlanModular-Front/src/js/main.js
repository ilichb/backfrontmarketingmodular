...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(`index.php?option=com_ajax&plugin=pluginMicroservicios&format=json&token=${token}`, {
        method: 'GET'
    })
    ...
    .then(data => {
        ...
        let datosCompletos = data.data && JSON.parse(data.data[0]);

        datosCompletos.servicios.map(s => {
            servicesArray.push({service: s.nombre, subServices: []});
        });

        datosCompletos.microservicios.forEach(m => {
            servicesArray.map(s => {
                if(s.service === m.servicio){
                    s.subServices.push(m.nombre)
                }
            })
        });

        let sectorObjeto = datosCompletos.sectorEconomico;

        for (let sector in sectorObjeto){
            let id = sectorObjeto[sector].nombre.split(' ').join('-');
            sectorComercialArray.push({id, name: sectorObjeto[sector].nombre});
        };
        console.log(sectorComercialArray);
        ...
        function generateServices(data, context, target){
            for(let i = 0; i < data.length; i++) {
                ...
                for(let j = 0; j < data[i].subServices.length; j++){
                    ...
                    button.addEventListener('click', function (){
                        if(selectedMicroServices[serviceId.split('-').join(' ')] && selectedMicroServices[serviceId.split('-').join(' ')].includes(microServiceId.split('-').join(' '))) {
                            selectedMicroServices[serviceId.split('-').join(' ')] = selectedMicroServices[serviceId.split('-').join(' ')].filter(microService => microService.split('-').join(' ') !== microServiceId.split('-').join(' '))
                        } else {
                            if(!selectedMicroServices[serviceId.split('-').join(' ')]) {
                                selectedMicroServices[serviceId.split('-').join(' ')] = [microServiceId.split('-').join(' ')];
                            }
                            else {
        ...
        signupButton.addEventListener('click', function (){
            ...
            if(name === null || name === '' || email === null || email === '') {
                ...
                formData.append('tipo', 'guardarDatos');
        ...
        submitButton.addEventListener('click', function () {
            ...
            for (let i = 0; i < actives.length; i++) {
                if(actives[i].classList.contains('detailed-sector-button')){
                    selectedCommercialSector = actives[i].id.split('-').join(' ');
                }
            ...
            if(selectedCommercialSector === null || selectedCountry === null || expenses === null || expenses === '' || roi === null || roi === '' || Object.keys(selectedMicroServices).length === 0) {
                ...
                fetch('index.php?option=com_ajax&plugin=pluginMicroservicios&format=json', {
                    ...
                    .then(data => {
                        ...
                        sales = dataMicroservices[0].sales;
                        salesPercentage.innerHTML = `${sales}%`;
